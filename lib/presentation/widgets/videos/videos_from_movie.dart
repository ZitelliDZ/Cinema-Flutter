...
class _VideosList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<_YouTubeVideoPlayer> videoPlayers = [];
    for (int i = 0; i < 2; i++) {
      if (i < videos.length) {
        videoPlayers.add(_YouTubeVideoPlayer(
            youtubeId: videos[i].youtubeKey, name: videos[i].name));
      }
    }
    return Column(
      ...
      children: [
        ...
        ...videoPlayers.map((video) => video).toList(),
      ],
    );
  }
}
